...
class BacaKomikAPI {
  ...
  async searchComics(queryParams) {
    try {
      const { title = '', author = '', year = '', status = '', order = 'title', type = '', genre = '' } = queryParams;
      const response = await axios.get(`${this.baseUrl}/comics`, {
        params: {
...
export default async function handler(req, res) {
  ...
  const { action, mangaId, title, author, genre } = queryParams;
      ...
      switch (action) {
        ...
        case 'details':
          if (mangaId) {
            ...
            return res.status(400).json({ error: 'Manga ID is required for details' });
          }
