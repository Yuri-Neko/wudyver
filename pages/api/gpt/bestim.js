...
export default async function handler(req, res) {
  ...
  try {
      ...
      messagesHistory: [
        {
          from: "chatGPT",
          content:
            assistant || "Saya AI dari OpenAI, diciptakan untuk membantu Anda mengeksplorasi ide, bertukar informasi, dan menyelesaikan masalah. Ada yang bisa saya bantu?",
        },
        {
          from: "you",
          content: message,
    ...
    const response = await axios.post(
      "https://chatgpt.bestim.org/chat/send2/",
      data,
      { headers }
    );

    const result = response.data || (
      await axios.post("https://chatgpt.bestim.org/chat/send/", data, {
        headers,
      })
    ).data;
    ...
    return res.status(500).json({ error: "Error occurred in TalkAI" });
  }
}
