...
export default async function handler(req, res) {
  ...
  if (!your_qus) {
    return res.status(400).json({ error: "Your question (prompt) is required" });
  }
  ...
  try {
    ...
    const response = await axios.post(Baseurl + "v1/chat/gpt/", {
      ...
      headers: {
        ...
        "X-Forwarded-For": Array.from({ length: 4 }, () => Math.floor(256 * Math.random())).join("."),
    ...
    return res.status(500).json({ error: "Error occurred during GPT-3 request" });
  }
}
