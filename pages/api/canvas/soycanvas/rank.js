...
export default async function handler(req, res) {
  if (req.method === "GET") {
    ...
    try {
      const rankImage = await new soycanvas.Rank().setAvatar(avatar).setBackground("image", background).setUsername(username).setBorder(borderColor).setStatus(status).setLevel(level).setRank(rank).setCurrentXp(currentXp).setRequiredXp(requiredXp).build();
