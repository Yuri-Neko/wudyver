...
export default async function handler(req, res) {
  if (req.method === "GET") {
    ...
    try {
      const securityImage = await new soycanvas.Security().setAvatar(avatar).setBackground("image", background).setCreatedTimestamp(Number(userTime)).setSuspectTimestamp(Number(suspectTime)).setBorder(borderColor).setLocale(locale).setAvatarBorder(borderColor).setOverlayOpacity(Number(overlayOpacity)).build();
