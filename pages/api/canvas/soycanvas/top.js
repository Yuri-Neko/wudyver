...
export default async function handler(req, res) {
  if (req.method === "GET") {
    ...
    try {
      const topImage = await new soycanvas.Top().setOpacity(opacity).setScoreMessage(scoreMessage).setAbbreviateNumber(abbreviateNumber).setBackground("image", background).setColors(colors).setUsersData(usersData).build();
