...
export default async function handler(req, res) {
  if (req.method === "GET") {
    ...
    try {
      const welcomeImage = await new soycanvas.WelcomeLeave().setAvatar(avatar).setBackground("image", background).setTitle(title).setDescription(description).setBorder(borderColor).setAvatarBorder(avatarBorderColor).setOverlayOpacity(overlayOpacity).build();
