...
export default async function handler(req, res) {
  if (req.method === "GET") {
    ...
    if (!username || !avatar) return res.status(400).json({
      error: "Missing required parameters"
    });
    try {
      const rankImage = await new soycanvas.oldRank().setAvatar(avatar).setBackground("image", background).setUsername(username).setBorder(borderColor).setStatus(status).setLevel(level).setRank(rank).setCurrentXp(currentXp).setRequiredXp(requiredXp).build();
      Promise.resolve(rankImage).then(() => {
