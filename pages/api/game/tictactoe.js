...
export default async function handler(req, res) {
  const { action, gameId, box } = req.query;
  ...
  try {
    ...
    switch (action) {
      case "create": {
        ...
        const existingGame = await TicTacToe.findOne({ gameId: id });
        if (existingGame) return res.status(400).json({ message: "Game ID already exists" });
      ...
      case "move": {
        ...
        if (!gameId || !box) return res.status(400).json({ message: "Missing required fields" });
        ...
        if (game.winner) return res.status(400).json({ message: "Game already finished" });
        if (game.board[row][col]) return res.status(400).json({ message: "Cell already occupied" });
        ...
        await game.save();
        return res.status(200).json({ message: "Move made successfully", game });
      }
      ...
      case "reset": {
        ...
        await game.save();
        return res.status(200).json({ message: "Game reset successfully", game });
      }
