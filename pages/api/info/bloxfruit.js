...
export default async function handler(req, res) {
    ...
    const { method, query: { type, item } } = req;
    ...
    try {
        switch (type) {
            ...
            case "details":
                if (!item) {
                    return res.status(400).json({ error: "Item parameter is required for details." });
                }
                return res.status(200).json(await bloxFruit.getDetails(item));
            case "trades":
                if (!item) {
                    return res.status(400).json({ error: "Item parameter is required for trades." });
                }
                return res.status(200).json(await bloxFruit.getTrades(item));
            default:
                return res.status(400).json({ error: "Invalid type parameter. Use 'stock', 'value-list', 'details', or 'trades'." });
        }
    } catch (error) {
