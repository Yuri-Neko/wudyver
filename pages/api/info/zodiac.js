...
export default async function handler(req, res) {
  const { method } = req;
  const { zodiac } = method === "GET" ? req.query : req.body;

  const invalidResponse = !zodiac || !validZodiacs.includes(zodiac.toLowerCase())
    ? res.status(400).json({
  ...
  try {
    const { data } = await axios.get(`https://www.fimela.com/zodiak/${zodiac}`);
    ...
    const result = {
      ...
      love: $("div:nth-child(3) > div.zodiak--content__content > p")
        .text()
        .replace("Couple", "\n\n- Couple")
