...
class ChatfaiAPI {
  ...
  async getConversationId(characterId = "zJRCoXBaX53HLo3FtuNb") {
    try {
      const response = await this.client.post('/conversations', { characters: [characterId] });
  ...
  async getMessages(conversationId = "ZzsWq8XaiS0nJVbfTocM", count = 10) {
    try {
      const response = await this.client.get(`/conversations/${conversationId}/messages`, { params: { count } });
  ...
  async waitForLatestMessage(conversationId = "ZzsWq8XaiS0nJVbfTocM", currentMessageId, timeout = 10000) {
    try {
  ...
  async sendAndRetrieve(prompt, characterId = "zJRCoXBaX53HLo3FtuNb", timeout = 10000) {
    try {
  ...
  async searchCharacters(query, perPage = 20) {
    try {
      const response = await this.client.get(`/characters/search`, { params: { q: query, per_page: perPage } });
      return response.data?.data;
    } catch (error) {
...
export default async function handler(req, res) {
  try {
    const { action = "chat", prompt, cid: characterId, timeout, query } = req.method === "GET" ? req.query : req.body;

    if (!action) {
      return res.status(400).json({ error: "action diperlukan." });
    }
    ...
    if (!authToken) {
      return res.status(500).json({ error: "CHATFAI_API_TOKEN tidak ditemukan dalam environment." });
    }
    ...
    if (action === "search") {
      if (!query) {
        return res.status(400).json({ error: "query diperlukan untuk pencarian." });
      }
    ...
    if (action === "chat" && prompt) {
      const latestMessage = await api.sendAndRetrieve(prompt, characterId || "zJRCoXBaX53HLo3FtuNb", timeout || 10000);
      return res.status(200).json({ success: true, message: latestMessage });
    }
