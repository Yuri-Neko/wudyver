...
class HeuristAi {
  ...
  async getModels() {
    try {
      const response = await axios.get(this.modelUrl, { headers: this.headers });
      return response.data;
    } catch (error) {
  ...
  if (req.method === 'GET' || req.method === 'POST') {
    if (action === 'chat') {
      ...
      try {
        ...
        return res.status(200).json({ result: parseResponse(chatResponse) });
      } catch (error) {
        return res.status(500).json({ error: 'Error in chat request' });
      }
