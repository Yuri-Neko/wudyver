...
class Heurist {
  ...
  async generateImage({ prompt = "Master", neg_prompt = "(worst quality: 1.4), bad quality, nsfw", num_iterations = 25, guidance_scale = 5, width = 512, height = 768, seed = -1, model = "BrainDance" }) {
    const payload = [
      {
        prompt,
        neg_prompt,
    ...
    try {
      const response = await axios.post(`${this.url}${model}`, payload, { headers: this.headers });
      return { parsed: parseResponse(response.data), response: response.data };
    } catch (error) {
