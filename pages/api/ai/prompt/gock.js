...
class Gock {
  ...
  async flux(prompt, model = 'groq:llama-3.1-70b-versatile') {
    ...
    try {
      const response = await axios.post(`${this.baseUrl}/flux`, body, { headers: this.headers });
  ...
  async html(prompt, model = 'groq:llama-3.3-70b-versatile') {
    ...
    try {
      const response = await axios.post(`${this.baseUrl}/html-generator`, body, { headers: this.headers });
...
export default async function handler(req, res) {
  const { type = 'flux', prompt, model } = req.method === 'GET' ? req.query : req.body;
  const gock = new Gock();
