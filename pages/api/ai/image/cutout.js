...
class Cutout {
  ...
  async createJob(prompt, style, quantity = 3, width = 512, height = 512) {
    ...
    const data = { prompt, style, quantity, width, height };
    try {
...
export default async function handler(req, res) {
  const { prompt, style, quantity, width, height } = req.method === 'POST' ? req.body : req.query;

  if (req.method === 'POST' || req.method === 'GET') {
    try {
      const cutout = new Cutout();
      const taskData = await cutout.generate(
