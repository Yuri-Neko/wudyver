...
export default async function handler(req, res) {
  const { prompt, aspect_ratio = "3:2", ver = "v1" } = req.method === "GET" ? req.query : req.body;

  if (!prompt) return res.status(400).json({ error: 'Prompt tidak boleh kosong' });

  try {
    const response = await axios.post(
      "https://www.gening.ai/cgi-bin/auth/aigc/image",
      { prompt, ver, aspect_ratio },
      {
        headers: {
          accept: "*/*",
    ...
    if (response.data.code !== 0) return res.status(500).json({ error: 'Gagal membuat gambar' });

    const { id, index } = response.data.data;

    let statusResponse;
    while (!statusResponse || statusResponse.data.code !== 0 || statusResponse.data.data.result.msg !== "success") {
      statusResponse = await axios.get(
        `https://www.gening.ai/cgi-bin/auth/aigc/status?id=${id}&index=${index}&type=image_normal`,
        {
          headers: {
            accept: "*/*",
            "accept-language": "id-ID,id;q=0.9",
