...
class Koala {
  ...
  async generate(payload) {
    ...
    try {
      ...
      const parsedData = lines.map((line) => line.slice(5)).map((text) => text.slice(1, -1)).reduce(
  (acc, text) => {
...
export default async function handler(req, res) {
  ...
  const { prompt, model } = req.method === "GET" ? req.query : req.body;
  ...
  try {
    const response = await koala.generate({ prompt });
