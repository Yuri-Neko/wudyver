...
export default async function handler(req, res) {
  try {
    const { url: img, prompt = "Analyze this image in detail, including its main elements, style, composition, and any notable features." } = req.method === 'GET' ? req.query : req.body;
    if (!img) {
    ...
    if (img.startsWith('http://') || img.startsWith('https://')) {
      ...
      return res.status(400).json({ error: 'Hanya mendukung URL gambar' });
    }
