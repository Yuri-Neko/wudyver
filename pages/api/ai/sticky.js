...
class Sticky {
  ...
  async generateImage(prompt, num) {
    ...
    const data = { prompt, n: num, response_format: 'url' };
    try {
      const response = await axios.post(url, data, { headers: this.headersGenerate });
  ...
  async sendImageData(imgList) {
    ...
    const data = { imgList: imgList.map(img => ({ url: img.url })) };
    try {
      const response = await axios.post(url, data, { headers: this.headersSticky });
      return response.data;
    } catch (error) {
      throw error;
    }
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    if (action === 'generate') {
      const { prompt, num } = req.query;
      if (!prompt || !num) {
        return res.status(400).json({ error: 'Missing prompt or num parameter' });
      }
      ...
      try {
        const result = await sticky.sendImageData([{ url }]);
        res.status(200).json(result);
      } catch (error) {
    ...
    if (action === 'generate') {
      ...
      try {
        const result = await sticky.sendImageData(imgList.map(img => ({ url: img.url })));
        res.status(200).json(result);
      } catch (error) {
        res.status(500).json({ error: 'Error sending image data' });
      }
