...
class ImagePrompt {
  ...
  async text(prompt = 'Men') {
    try {
      const body = JSON.stringify({ userPrompt: prompt, page: 'image-prompt-generator' });
  ...
  async edit(prompt = 'Men', detail = 'Very Men') {
    try {
      const body = JSON.stringify({ userPrompt: prompt, editDetails: detail });
...
export default async function handler(req, res) {
  ...
  const { action, url, model, lang, prompt, detail } = params;
