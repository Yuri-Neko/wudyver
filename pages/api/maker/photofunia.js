...
const proxyUrl = ["https://thingproxy.freeboard.io/fetch/", "https://cors.newfrontdoor.org/api/cors?url=", "https://api.allorigins.win/raw?url="];
...
const search = async teks => {
  try {
    const response = await fetch(`${proxyBase + baseM}/search?q=${encodeURIComponent(teks)}`);
