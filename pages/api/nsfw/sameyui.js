...
export default async function handler(req, res) {
  const { method } = req;
  const { action, query, sort = 'latest', page = 1, id, type = 'latest' } = method === 'GET' ? req.query : req.body;
  ...
  try {
    switch (action) {
      case 'home':
        ...
        return res.status(200).json({ result: homeResult });
      ...
      case 'related':
        ...
        return res.status(200).json({ result: relatedResult });
