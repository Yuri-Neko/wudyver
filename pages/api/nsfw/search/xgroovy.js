...
class XGroovy {
  ...
  async search(query = "") {
    ...
    try {
      const {
        data
      } = await axios.get(randomProxyUrl + `${this.baseUrl}/search/${encodeURIComponent(query)}/`);
  ...
  async photoai() {
    try {
      const {
        data
      } = await axios.get(randomProxyUrl + `${this.baseUrl}/photos/categories/ai/`);
      const $ = cheerio.load(data);
      return {
  ...
  async photo() {
    try {
      const {
        data
      } = await axios.get(randomProxyUrl + `${this.baseUrl}/photos/`);
      const $ = cheerio.load(data);
      return {
