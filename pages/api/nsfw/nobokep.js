...
export default async function handler(req, res) {
  ...
  if (action === 'search') {
    if (!query) {
      return res.status(400).json({ error: 'Query parameter is required for search action' });
    }
  ...
  if (action === 'detail') {
    if (!url) {
      return res.status(400).json({ error: 'URL parameter is required for detail action' });
    }
    const scraper = new WebScraper();
    const resultDetail = await scraper.detail(url);
    return res.status(200).json({ result: resultDetail });
  }

  return res.status(400).json({ error: 'Invalid action or missing query/url parameter' });
}
