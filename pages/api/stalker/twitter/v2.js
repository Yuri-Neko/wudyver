...
export default async function handler(req, res) {
  ...
  if (!username) {
    return res.status(400).json({ error: 'Etdaahhh, mana username twitter nya! mada iya mau ngecek kagak ada username nya 🗿' });
  }
  ...
  try {
    ...
    if (!data || Object.keys(data).length === 0) {
      return res.status(404).json({ error: 'Kagak ada datanya .. check yang lain aja dahh 😶' });
    }
