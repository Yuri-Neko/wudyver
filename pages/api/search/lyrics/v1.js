...
class Genius {
  ...
  async search(query) {
    ...
    try {
      const response = await axios.get(this.randomProxyUrl + url.toString(), { 
        headers: this.headers
      });
      return response.data.response.hits.filter((e) => e.type === 'song')
        .map(({ result }) => ({
  ...
  async lyrics(url) {
    try {
      const response = await axios.get(`${this.randomProxyUrl}${url}`, { 
        headers: this.headers
      });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'lyrics') {
      if (!url) return res.status(400).json({ error: 'URL parameter is required for lyrics.' });
    ...
    if (query) {
      ...
      return res.status(200).json({ song: songs[0], lyrics });
    }

    return res.status(400).json({ error: 'Invalid request. Provide an action, query, or URL.' });
  } catch (error) {
