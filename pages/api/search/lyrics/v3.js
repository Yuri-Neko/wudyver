...
class Lirik {
  ...
  async search(query) {
    try {
      const { data } = await axios.get(this.randomProxyUrl + `https://www.lirikterjemahan.id/search?q=${encodeURIComponent(query)}`);
  ...
  async lyrics(url) {
    try {
      const { data } = await axios.get(this.randomProxyUrl + url);
    const $ = cheerio.load(data);
