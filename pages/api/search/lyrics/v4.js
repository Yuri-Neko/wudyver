...
class Musix {
  ...
  async getToken() {
    ...
    this.tokenData = { user_token: userToken, expiration_time: Math.floor(Date.now() / 1000) + 600 };
  }

  async ensureToken() {
    if (!this.tokenData || this.tokenData.expiration_time < Math.floor(Date.now() / 1000)) {
      await this.getToken();
    }
  }
  ...
  async searchTrack(query) {
    ...
    return { id: track.track_id, details: response };
  }
  ...
  async getLyrics(trackId) {
    ...
    return { lyrics, details: response };
  }
}

export default async function handler(req, res) {
  const { action, query, id } = req.method === 'GET' ? req.query : req.body;
  const musix = new Musix();

  try {
    if (action === 'search') {
      if (!query) return res.status(400).json({ error: 'Query parameter is required for search.' });
      const track = await musix.searchTrack(query);
      return res.status(200).json({ result: track });
    }

    if (action === 'lyrics') {
      if (!id) return res.status(400).json({ error: 'ID parameter is required for lyrics.' });
      const lyrics = await musix.getLyrics(id);
      return res.status(200).json({ result: lyrics });
    }

    if (query) {
      const track = await musix.searchTrack(query);
      const lyrics = await musix.getLyrics(track.id);
      return res.status(200).json({ track, lyrics });
    }

    return res.status(400).json({ error: 'Invalid request. Provide an action, query, or ID.' });
  } catch (error) {
