...
export default async function handler(req, res) {
  const { action, query, artist, title } = req.method === "GET" ? req.query : req.body;
  ...
  try {
    ...
    if (action === 'lyrics') {
      if (!(artist && title)) return res.status(400).json({ error: 'Artist and title parameters are required for lyrics.' });
    ...
    return res.status(400).json({ error: 'Invalid request. Provide an action, query, or valid parameters.' });
  } catch (error) {
