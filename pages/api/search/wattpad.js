...
class Wattpad {
  ...
  async viewParts(id) {
    try {
      const params = {
        fields:
          "id,title,url,modifyDate,wordCount,photoUrl,commentCount,voteCount,readCount,voted,pages,text_url,rating,group(id,title,cover,url,user(username,name,avatar,twitter,authorMessage),rating,parts(title,url,id)),source(url,label)",
      };
...
export default async function handler(req, res) {
  ...
  try {
    const { action, id, query: searchQuery, limit = 20, offset = 0, mature = true, language = "20" } = query;
    let result;

    if (method === "GET") {
      if (action === "search" && searchQuery) {
        result = await wattpad.search(searchQuery, limit, offset, mature, language);
      } else if (action === "detail" && id) {
        result = await wattpad.detail(id);
      } else if (action === "viewParts" && id) {
        result = await wattpad.viewParts(id);
      } else if (action === "read" && id) {
        result = await wattpad.read(id);
      } else {
