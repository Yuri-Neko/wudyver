...
class Jkt48Scraper {
  ...
  async getLatestNews() {
    ...
    $('.card-goods').each((_, element) => {
      ...
      goods.push({ title, link: fullLink, image: imageSrc });
    });
  ...
  async getEvents() {
    ...
    $('.entry-schedule__calendar .table tbody tr').each((_, element) => {
      ...
      $(element).find('.contents').each((_, eventElement) => {
        ...
        events.push({ event, link: `${this.baseUrl}${link}` });
      });
  ...
  async getNewsList() {
    ...
    $('.entry-news__list').each((_, element) => {
      ...
      newsList.push({ title, link: link ? `${this.baseUrl}${link}` : null, date });
    });
  ...
  async getCalendarEvents() {
    ...
    $('.entry-schedule__calendar table tbody tr').each((_, element) => {
      ...
      calendarEvents.push({ date: dateText, events });
    });
...
export default async function handler(req, res) {
  ...
  const { action, url } = req.method === "GET" ? req.query : req.body;
