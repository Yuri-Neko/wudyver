  ...
  async detail(url) {
    ...
    try {
      ...
      $(".vue--security-severity-table tr").each((_, element) => {
        ...
        if (version) versionData.push({ version, vulnerabilities, licenseRisk, severityData });
      });
...
export default async function handler(req, res) {
  const { action, query, url, source } = req.query;

  const scraper = new SynkScraper(source || "npm");

  try {
    if (action === "search") {
      if (!query) return res.status(400).json({ error: "Parameter 'query' diperlukan untuk pencarian." });
      const results = await scraper.search(query);
      return res.status(200).json(results);
    }

    if (action === "detail") {
      if (!url) return res.status(400).json({ error: "Parameter 'url' diperlukan untuk detail." });
      const details = await scraper.detail(url);
      return res.status(200).json(details);
    }

    res.status(400).json({ error: "Aksi tidak valid. Gunakan 'search' atau 'detail'." });
  } catch (error) {
