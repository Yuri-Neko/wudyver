...
export default async function handler(req, res) {
  const { query: q } = req.method === "GET" ? req.query : req.body;
  ...
  try {
    ...
    const thumbnailUrls = $('script[type="application/ld+json"]')
      .toArray()
      .map((element) => {
        try {
    ...
    res.status(200).json({ result: thumbnailUrls });
  } catch (error) {
    console.error("Failed to fetch data from Storyset:", error.message);
    res.status(500).json({ error: "Failed to fetch data from Storyset" });
  }
}
