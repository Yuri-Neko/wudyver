...
export default async function handler(req, res) {
  if (req.method === "GET") {
    ...
    if (!expr) {
      return res.status(400).json({ error: "Parameter 'expr' diperlukan." });
    }

    try {
      const response = await axios.get(mathJsApi, {
        params: { expr, precision },
      });
      return res.status(200).json({ result: response.data });
    } catch (error) {
      return res
        .status(error.response?.status || 500)
        .json({ error: error.response?.data || "Terjadi kesalahan." });
    }
  } else if (req.method === "POST") {
    const { expr, precision } = req.body;

    if (!expr) {
      return res.status(400).json({ error: "Field 'expr' diperlukan." });
    }
