...
export default async function handler(req, res) {
  const { method } = req;
  const { code = `console.log('hello')`, lang: language = 'javascript', service = '2' } = method === "POST" ? req.body : req.query;
  ...
  try {
    const result = service === '1' ? await compiler.runoob(code, language) : service === '2' ? await compiler.leez(code) : res.status(400).json({ error: "Invalid service" });
