...
class TikVid {
    ...
    async convert(vid, audio, image, exp, token, url) {
        const params = new URLSearchParams({ftype: 'mp4', v_id: vid, audioUrl: audio, audioType: 'audio/mp3', imageUrl: image, fquality: '1080p', fname: 'TikVid.io', exp, token});
        try {
            const { data } = await axios.post(url, params, { headers: this.headers });
            return data;
        } catch (error) {
            console.error(error);
            return null;
    ...
    job(jobId) {
        return new Promise((resolve, reject) => {
            const ws = new WebSocket(`wss://s2.tik-cdn.com/sub/${jobId}?fname=TikVid.io`, { headers: { 'Origin': 'https://tikvid.io', 'User-Agent': this.headers['user-agent'] } });
            ws.on('message', (data) => {
    ...
    async download(url) {
        if (!this.regex.test(url)) return { error: "Link TikTok tidak valid. Coba gunakan link TikTok yang lain." };
        try {
            const { data } = await axios.post(`${this.link}/api/ajaxSearch`, new URLSearchParams({ q: url, lang: 'en' }), { headers: this.headers });
            const $ = cheerio.load(data.data);
            const result = {
                title: $('.tik-video .content h3').text().trim(),
                thumbnail: $('.image-tik img').attr('src'),
                downloads: { video: {}, images: [], audio: null }
            };
            ...
            if (exp && token && convertUrl) result.convert = { exp, token, convertUrl };
            $('.photo-list .download-items').each((_, item) => {
                const $item = $(item);
                result.downloads.images.push({ thumbnail: $item.find('img').attr('src'), dlink: $item.find('a').attr('href') });
            });
            ...
            return { error: "Terjadi kesalahan. Coba lagi beberapa saat lagi." };
        }
    }
}
...
export default async function handler(req, res) {
    ...
    if (!url) {
        return res.status(400).json({ error: 'URL tidak ditemukan. Pastikan URL TikTok sudah benar.' });
    }
    ...
    try {
        ...
        res.status(500).json({ error: "Terjadi kesalahan pada server, coba lagi nanti.", details: error.message });
    }
}
