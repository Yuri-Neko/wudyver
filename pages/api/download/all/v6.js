...
class AioDown {
  ...
  async getDownloadLinks(videoUrl, desiredQuality = "360p") {
    try {
...
const aiodown = async (videoUrl, desiredQuality = "360p") => {
...
export default async function handler(req, res) {
  ...
  if (!(url || quality)) return res.status(400).json({
    message: "No url, quality provided"
  });
  const result = await aiodown(url, quality);
